...
def test_rv_begin_is_midpoint_of_dry_wet_transition():
    ...
    assert t == {'beginInMin': 15, 'endInMin': None, 'intensityDbz': ON + 5}
...
def test_rv_no_begin_when_all_dry():
    ...
    assert derive_rv_timing(samples, False) == {'beginInMin': None, 'endInMin': None, 'intensityDbz': None}
...
def test_rv_end_is_midpoint_of_wet_dry_transition():
    ...
    assert t == {'beginInMin': None, 'endInMin': 15, 'intensityDbz': None}


def test_rv_no_end_when_wet_throughout():
    samples = [_s(10, ON + 5), _s(20, ON + 5), _s(30, ON + 5)]
    assert derive_rv_timing(samples, True) == {'beginInMin': None, 'endInMin': None, 'intensityDbz': None}


def test_rv_empty_or_none():
    assert derive_rv_timing(None, False) == {'beginInMin': None, 'endInMin': None, 'intensityDbz': None}
    assert derive_rv_timing([], False) == {'beginInMin': None, 'endInMin': None, 'intensityDbz': None}
...
def test_consensus_disagreement_caps_high_at_med():
    ...
    assert out == {'minutes': 30, 'confidence': 'med', 'method': 'edge', 'intensityDbz': None}
...
def test_rv_only_fills_method_none_at_med():
    ...
    assert out == {'minutes': 25, 'confidence': 'med', 'method': 'rv', 'intensityDbz': None}
